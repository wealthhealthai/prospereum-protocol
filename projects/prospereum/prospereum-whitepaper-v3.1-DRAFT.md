...
# Prospereum — Whitepaper v3.1

---

PROSPEREUM Protocol (PSRE)
Proof of Net Retention
White Paper v3.1 (March 2026)
...
Partners must make an initial PSRE purchase of at least $S_{min}$ (denominated in USDC) at
the time of PartnerVault creation. The recommended minimum is equivalent to $500 USDC.
...
This replaces the separate vault bond mechanism used in earlier versions. The initial buy is
not refundable and not rewarded, making it strictly more punitive than a refundable bond.

The factory contract checks that the USDC-equivalent input satisfies $\geq S_{min}$ before
deploying the vault, protecting against price volatility manipulation.

**Vault Expiry**

PartnerVaults with no cumulative high-water-mark growth ($\Delta\text{cumS}_p = 0$) for
4 consecutive epochs are automatically marked inactive. Inactive vaults are excluded from
epoch reward computation, freeing their registry slot.
...
This is the core mechanism of v3.1: rewards are based not on gross buying activity, not on
...
### 6.5 Ongoing Epoch Reward

After first qualification (see Section 6.6), the per-epoch reward is:

$$\text{reward}_p(t) = \max\!\left(0,\ r_{base} \times \big(\text{cumS}_p(t) - \text{cumS}_p(t-1)\big)\right)$$

If the ecosystem did not grow past its prior cumS peak in epoch $t$, then
$\text{cumS}_p(t) = \text{cumS}_p(t-1)$ and the reward is zero.
...
$$\text{reward}_{p,\text{first}} = r_{base} \times \big(\text{cumS}_p(M) - S_p(N)\big)$$
...
This is the sum of all positive cumS growth increments across all partners in the epoch.
Partners with flat or declining ecosystem balances contribute zero to $E_{demand}$.
...
cumS growth increments:

$$R_p(t) = (1-\theta) \cdot R_p(t-1) + \theta \cdot \Delta\text{cumS}_p(t)$$

where:

$$\Delta\text{cumS}_p(t) = \max\!\left(0,\ \text{cumS}_p(t) - \text{cumS}_p(t-1)\right)$$
...
$$\text{reward}_{p,\text{raw}}(t) = r_p(t) \times \Delta\text{cumS}_p(t)$$
...
- **More punitive than a refundable bond.** A bond is locked but eventually returned; the
  initial buy pays DEX fees, slippage, and irrecoverable opportunity cost with zero reward.
- **No spam incentive.** A spammer who registers hundreds of vaults pays $S_{min}$ per vault
  with zero reward on each initial buy. Combined with the scarcity cap and the need to exceed
...
The reward formula rewards only $\Delta\text{cumS}_p > 0$:
...
- **Real growing commerce naturally grows cumS.** A partner with an active and expanding
  customer base will continuously acquire new customers, distribute PSRE, and grow their
  ecosystem balance. Their cumS grows continuously. Reward flows naturally.
- **Slow seasons are accommodated without penalty.** A partner whose ecosystem goes flat
  during a slow season earns zero reward — but their cumS does not reset. When growth
  resumes, reward flows immediately. Long-term program maintenance is encouraged without
...
Prospereum v3.1 introduces a structurally self-reinforcing defense against gaming:
...
The initial buy at vault creation earns zero reward. Vault spammers pay irrecoverable costs
(fees, slippage, and the opportunity cost of the initial buy) per vault with zero return on the
creation transaction. This is strictly more punitive than a refundable bond.
...
PartnerVaults with no cumS growth for 4 consecutive epochs are automatically marked
inactive, preventing ghost vault accumulation in the registry.
...
rewards. For every 100 PSRE a partner ecosystem grows (raising cumS by 100 PSRE):

- **100 PSRE is removed from circulating supply** (bought from market, held in vault ecosystem)
- **Only ~10 PSRE of new supply is emitted as reward** (at $r_{base} = 10\%$ applied to $\Delta\text{cumS}$)
- That reward PSRE **also enters the vault ecosystem** — it is distributed to CustomerVaults, not sold

The net circulating supply effect per epoch of partner ecosystem growth is approximately
**−90%**: for every 100 PSRE of cumS growth, only ~10 PSRE of new supply is created, and
both the purchased PSRE and the reward enter the vault ecosystem rather than circulating.

### 11.3 Reward PSRE Does Not Circulate
...
Prospereum v3.1 refines **Proof of Net Retention** with a cleaner, more structurally sound
reward mechanism: the cumulative high-water-mark.

Where v3.0 rewarded time-weighted retention, v3.1 rewards only cumulative net ecosystem
growth — the delta between an ecosystem's current balance and its all-time peak. This single
architectural change eliminates the TWR-delta formula, the 4-epoch vesting schedule, and
the vault bond, replacing all three with a single, self-enforcing ratchet mechanism.

The result is a protocol that is:

**Demand-Bounded ∩ Scarcity-Controlled ∩ Growth-Verified ∩ Governance-Limited**
...
should be recognized permanently and rewarded only when it grows further.
