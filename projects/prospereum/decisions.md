...
| PartnerVaultFactory v4 router gap | Factory hardcodes Uniswap v3 SwapRouter (0x2626664c...). v4 launched June 2025. If v3 liquidity migrates to v4, createVault() swaps will fail. Fix: deploy PartnerVaultFactory v2 with v4 UniversalRouter support. Genesis LP seeded to v3 pool to unblock launch. Flag for first factory upgrade. | Jason | 2026-04-29 |

| PSRE-native partner entry | PartnerVaultFactory.createVault() and PartnerVault.buy() now accept PSRE directly — no USDC, no router, no DEX swap. Partners source PSRE via any channel (Uniswap, Aerodrome, CEX, OTC) before calling. Eliminates hardcoded Uniswap v3 router dependency. psreMin replaces S_MIN (default 5000 PSRE ≈ $500 at launch). RewardEngine.setFactory() added to support factory redeploys without full RE upgrade. 237/237 unit tests passing. Commit f4d4cc6. | Shu + Jason | 2026-04-30 |

| PSRE-native delta audit — CLEARED | Nadir Ali / BlockApex reviewed commit f4d4cc6 (PSRE-native refactor). Issued 6 observations. All fixed in 0aba2e9. Nadir confirmed "all good to proceed" May 8 2026. Full audit trail in audit/blockapex-findings-response.csv (PSRE-Delta #1–#6). Protocol now fully audited through commit 0aba2e9. | Nadir / BlockApex | 2026-05-08 |
