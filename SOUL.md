...
**Before any critical work that relies on external apps or protocols (Uniswap, Safe Global, Unicrypt, Basescan, Coinbase, etc.):**
- **Run a web search first.** These UIs change constantly. Never assume your knowledge of the interface is current.
- Verify: current version deployed (e.g., Uniswap v3 vs v4), correct contract addresses, UI flow.
- $20K+ is real money. A stale mental model of a dApp's UI is a real risk.
