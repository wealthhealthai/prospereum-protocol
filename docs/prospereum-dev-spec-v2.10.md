...
2.2 PartnerNFT (identity) 
• Mintable by anyone (permissionless). 
• 1 per partner entity (enforced by protocol rules if desired). 
• Non-transferable or transfer-restricted recommended. 
• Maps partnerId -> owner. 
2.3 PartnerVaultFactory 
• Creates a PartnerVault for a PartnerNFT owner. 
• partnerId -> vaultAddress mapping. 
• Ensures one vault per partnerId. 
2.4 PartnerVault 
...
• address partnerOwner (or partnerId) 
...
2.5 StakingVault (includes LP staking) 
...
o stakePSRE(amount) 
o stakeLP(amount) 
• Convert LP amount into PSRE-equivalent: 
stakeTime = stakeAmount × stakingDuration 
2.6 RewardEngine (combined emission + reward vault) 
...
• split params 
Partner accounting (by partnerId): 
• uint256 cumBuySnapshot[partnerId] optional cache 

---
_Page 6_

• uint256 creditedNB[partnerId] 
• uint256 R[partnerId] (rolling score, 1e18-scaled PSRE units) 
Global rolling sum: 
• uint256 sumR; (sum of all R values; updated incrementally) 
...
• partner reward owed: owedPartner[partnerId] 
...
claimPartner(uint256 epochId, uint256 partnerId) 
...
Inputs: 
• partnerVault = vaultOf(partnerId) 
• cumBuy = PartnerVault.cumBuy() 
Definitions: 
• NB = cumBuy (since sell disabled) 
• deltaNB = max(0, NB - creditedNB[partnerId]) 
• update: creditedNB[partnerId] += deltaNB 
...
Implement similarly for LP stake with weight factor. 
...
mint outside finalizeEpoch P-based minting rule 
Add a timelock for any parameter updates. 
 
10. Events (required) 
Emit events for indexers and audits: 
• EpochFinalized(epochId, B, E_demand, E_scarcity, B_partners, B_stakers, minted) 
• PartnerDeltaComputed(epochId, partnerId, deltaNB, alpha_p, weight, reward) 
• StakeClaimed(epochId, user, stakeTime, reward) 
• PartnerClaimed(epochId, partnerId, reward) 
• PartnerBought(partnerId, amountIn, psreOut, cumBuy) 
• Distributed(partnerId, to, amount) 
 
11. Required Invariants (assertions) 
Always enforce: 
• T <= S_EMISSION 
• creditedNB[p] <= cumBuy[p] 
• deltaNB >= 0 
...
2. PartnerNFT + PartnerVaultFactory + PartnerVault buy/distribute 
