...
# Prospereum Developer Specification v3.2

PROSPEREUM (PSRE)
Developer Specification v3.2

---

## 0. Design Decisions Locked for v3.2
...
- **Distribution-to-customer-vaults as reward trigger is eliminated.** Only net ecosystem
  growth (effectiveCumS increment) earns reward.
...
- **effectiveCumS deduction (v3.2 addition):** reward PSRE is excluded from the reward basis
  via `effectiveCumS_p(t) = cumS_p(t) - cumulativeRewardMinted[vault]`. This prevents
  reward compounding.
- Reward per epoch: $r_{base} \times \max(0,\ \text{effectiveCumS}_p(t) - \text{effectiveCumS}_p(t-1))$.
...
- $E_{demand}(t) = r_{base} \times \sum_p \max(0,\ \text{effectiveCumS}_p(t) - \text{effectiveCumS}_p(t-1))$.
...
### 1.3 Economic Parameters (v3.2 defaults + bounds)
...
$\Delta\text{effectiveCumS}_p$), not on absolute cumS.
...
These multipliers yield effective reward rates of 8% / 10% / 12% at r_base = 10%.
...
```
S_MIN               = 500e6   // $500 USDC, denominated in USDC base units (6 decimals)
                      // Factory validates: usdcAmountIn >= S_MIN (no oracle needed)
                      // DAO-adjustable within bounds.
...
// The delta that earns first reward = effectiveCumS_p(M) - initialCumS[vault]
...
- Dust stays unminted (scarcity-positive). Do not carry forward in v3.2.

### 1.9 When sumR == 0

In early epochs there may be no partners with $\Delta\text{effectiveCumS} > 0$.

```
if sumR == 0:
    s_p = 0 for all partners
    alpha_p = r_base for any partner with deltaEffectiveCumS_p > 0
```
...
1. Validates USDC input amount: `usdcAmountIn >= S_MIN` (S_MIN = 500e6). **Revert if
   insufficient.** No oracle needed — S_MIN is denominated directly in USDC.
...
**No vault bond is collected.** The initial PSRE buy IS the entry cost. S_MIN is enforced
in USDC terms (500e6) and requires no external price oracle.

**CustomerVault deployment:**

CustomerVault deployment gas is paid by the partner (the partnerOwner of the parent
PartnerVault). The factory deploys CustomerVaults as EIP-1167 minimal proxy clones on
behalf of the PartnerVault; the partner's transaction covers all gas.
...
- **CustomerVault deployment gas is paid by the partner (the partnerOwner of the parent
  PartnerVault).** The partner's transaction covers all deployment and initialization gas.
...
minimizes deployment gas for potentially thousands of CustomerVaults. All deployment
gas is borne by the partner.
...
**Responsibilities (v3.2 changes in bold):**

- Track total emitted $T$.
- Maintain epoch state.
- **Compute $\text{effectiveCumS}_p(t) = \text{cumS}_p(t) - \text{cumulativeRewardMinted}[vault]$ per partner vault per epoch.**
- **Compute $\Delta\text{effectiveCumS}_p$ per partner vault per epoch via `vault.snapshotEpoch()` plus the effectiveCumS deduction.**
...
- Compute EMA status (now on $\Delta\text{effectiveCumS}_p$ instead of TWR).
...
uint256 public k;                      // uint (immutable in v3.2)
uint256 public theta;                  // 1e18-scaled EMA factor
// tier thresholds + multipliers
// split params
// vault lifecycle params
uint256 public S_MIN;                  // 500e6 (USDC, 6 decimals) — no oracle needed
```
...
mapping(address => uint256) public cumulativeRewardMinted; // (v3.2) total rewards ever minted per vault
mapping(address => uint256) public lastEffectiveCumS;      // (v3.2) effectiveCumS stored after last epoch
mapping(address => uint256) public R;                      // EMA rolling score (tracks deltaEffectiveCumS)
...
mapping(uint256 => uint256) public epochDeltaEffectiveCumSTotal;  // sum of all deltaEffectiveCumS this epoch
...
1. Call `vault.snapshotEpoch()` → returns raw `deltaCumS_p` (cumS − lastEpochCumS).
2. Compute effectiveCumS and delta:
   ```
   effectiveCumS_p(t)   = lastEpochCumS[vault]  - cumulativeRewardMinted[vault]
   //  (lastEpochCumS was just updated by snapshotEpoch to vault's current cumS)
   effectiveCumS_p(t-1) = lastEffectiveCumS[vault]
   deltaEffectiveCumS   = max(0, effectiveCumS_p(t) - effectiveCumS_p(t-1))
   ```
3. Read `qualified[vault]` and `initialCumS[vault]`.
4. If `!qualified[vault]`:
   - Check if vault's current `cumS > initialCumS[vault]`.
   - If yes: mark `qualified[vault] = true`, and compute first reward:
     `firstRewardBasis = effectiveCumS_p(t) - initialCumS[vault]`
     `deltaEffectiveCumS = max(0, firstRewardBasis)`
   - If no: `deltaEffectiveCumS` contribution = 0 (initial buy epoch earns nothing).
5. If `qualified[vault]`: compute `rewardRaw_p = alpha_p * deltaEffectiveCumS_p / 1e18`.
6. Accumulate `deltaEffectiveCumSTotal += deltaEffectiveCumS_p`.
7. After computing reward: `cumulativeRewardMinted[vault] += rewardRaw_p`.
8. Store: `lastEffectiveCumS[vault] = effectiveCumS_p(t)`.
9. Update vault activity tracker.
...
if W > 0:
    for each qualifying vault p:
        raw_p = B_partners * (alpha_p * deltaEffectiveCumS_p / 1e18) / W
...
### 5.2 effectiveCumS Computation (v3.2)

After receiving `deltaCumS_p` from `snapshotEpoch()`, the RewardEngine computes the
effective reward basis using the `cumulativeRewardMinted` deduction:

```
// In RewardEngine.finalizeEpoch(), per vault:

effectiveCumS_p(t)   = cumS_p(t) - cumulativeRewardMinted[vault]
//  where cumS_p(t) = lastEpochCumS[vault] (just updated by snapshotEpoch)

effectiveCumS_p(t-1) = lastEffectiveCumS[vault]   // stored from prior epoch

deltaCumS = max(0, effectiveCumS_p(t) - effectiveCumS_p(t-1))

reward_p = r_base * tieredMultiplier(s_p) * deltaCumS / 1e18

cumulativeRewardMinted[vault] += reward_p  // update AFTER computing reward

lastEffectiveCumS[vault] = effectiveCumS_p(t)  // store for next epoch
```

**Key invariant:** `cumulativeRewardMinted[vault]` is incremented AFTER the reward for
the current epoch is computed. This ensures that the current epoch's reward does not
affect the current epoch's effectiveCumS calculation. The update propagates to the
next epoch naturally.

**Initialization:** At vault creation, `lastEffectiveCumS[vault] = initialCumS[vault]`
and `cumulativeRewardMinted[vault] = 0`.

### 5.3 Vault Activity Check
...
if currentCumS > initialCumS[vault]:
    ...
    // First reward basis uses effectiveCumS deduction:
    effectiveCumS_now = currentCumS - cumulativeRewardMinted[vault]
    // (cumulativeRewardMinted = 0 for first qualification since no rewards yet minted)
    // First reward = r_base * tieredMultiplier * (effectiveCumS_now - initialCumS[vault])
    emit VaultFirstQualified(vault, epochId, currentCumS, initialCumS[vault])
```

**Recommendation:** Store `lastEffectiveCumS[vault] = initialCumS[vault]` at vault creation,
so that the first epoch where cumS grows past initialCumS, `deltaEffectiveCumS_p = effectiveCumS_p(M) - initialCumS[vault]`,
which correctly reflects the first reward basis.

### 5.5 Rolling EMA Update (Partner Status)

EMA now tracks $\Delta\text{effectiveCumS}_p$ (effective cumS increment per epoch):

$$R_p(t) = (1-\theta) \cdot R_p(t-1) + \theta \cdot \Delta\text{effectiveCumS}_p(t)$$

Where $\Delta\text{effectiveCumS}_p(t) = \max(0,\ \text{effectiveCumS}_p(t) - \text{effectiveCumS}_p(t-1))$.

In code:

```
R_old = R[vault]
deltaEffectiveCumS_p = max(0, effectiveCumS_p(t) - lastEffectiveCumS[vault])

R_new = (R_old * (1e18 - theta) + deltaEffectiveCumS_p * theta) / 1e18
...
if s >= GOLD_TH:     m = M_GOLD     // 1.2e18
elif s >= SILVER_TH: m = M_SILVER   // 1.0e18
else:                m = M_BRONZE   // 0.8e18

alpha_p = (alphaBase * m) / 1e18
// Effective rates at r_base = 10%: Bronze = 8%, Silver = 10%, Gold = 12%
```

Unqualified vaults (where `!qualified[vault]`) use `deltaEffectiveCumS_p = 0` for EMA
update — they accumulate no contribution score until they qualify.
...
```
deltaEffectiveCumSTotal = Σ_p deltaEffectiveCumS_p   // sum of all qualifying partner increments

E_demand = alphaBase * deltaEffectiveCumSTotal / 1e18
// no division by EPOCH_DURATION — effectiveCumS is denominated in PSRE wei, not PSRE*seconds
```

**Key difference from v3.0:** $E_{demand}$ is now based on $\sum \Delta\text{effectiveCumS}_p$
(effective cumulative high-water-mark growth in PSRE wei, excluding reward PSRE), not on
$\Delta TWR$ (retention × time). This removes the EPOCH_DURATION divisor from the demand
formula and ensures rewards do not compound.
...
### 5.10 Partner Reward Distribution (No Vesting)

Per-partner weight (using tier-adjusted alpha and effectiveCumS delta):

$$w_p = \frac{\alpha_p \cdot \Delta\text{effectiveCumS}_p}{10^{18}}, \qquad W = \sum_p w_p$$
...
if W == 0:
    ...
    for each qualifying vault p:
        ...
        owedPartner[vault] += raw_p
        cumulativeRewardMinted[vault] += raw_p  // track for effectiveCumS deduction
```

No vesting schedule. `owedPartner[vault]` is immediately claimable by the vault owner via
`claimPartnerReward(vault)`.

### 5.11 Staker Reward Distribution (pull-based)
...
- EMA update uses $\Delta\text{effectiveCumS}_p$ per epoch only (not cumulative cumS).
...
- $E_{demand}$ is zero if no partner grew their effectiveCumS — no emission for flat ecosystem.
- Scarcity cap $E_{scarcity}(t)$ declines monotonically; no demand pressure can exceed it.
- `cumulativeRewardMinted[vault]` is strictly non-decreasing; rewards are never clawed back
  from the deduction.
- `cumulativeRewardMinted[vault]` is updated AFTER computing the current epoch's reward,
  preventing self-referential compounding within a single epoch.
...
- Spammers pay $S_{min}$ ($500 USDC) per vault with zero return on creation — no economic
  incentive to mass-create vaults.
- S_MIN is denominated in USDC (500e6) and validated directly by the factory — no oracle
  dependency required.
...
- Real partners with growing customer bases naturally grow effectiveCumS continuously. Reward
  flows naturally with genuine commerce activity.
- Slow seasons earn zero but do not forfeit past effectiveCumS. When growth resumes, rewards
  resume immediately, encouraging long-term program maintenance rather than abandonment.

### 9.6 Why effectiveCumS Deduction Prevents Reward Compounding (v3.2)

- Without the deduction: reward PSRE deposited into a vault increases ecosystemBalance →
  increases cumS → increases deltaCumS → generates more reward next epoch → feedback loop.
- With the deduction: `cumulativeRewardMinted[vault]` exactly cancels the cumS increase
  from reward deposits. effectiveCumS is unchanged by reward deposits. No compounding.
- The deduction requires no tag tracking, no dual balance system, and no PSRE labeling.
  A single monotonically increasing counter per vault is sufficient.
...
- Decrease `cumulativeRewardMinted` of any vault.
...
```solidity
// Epoch
EpochFinalized(epochId, B, E_demand, E_scarcity, B_partners, B_stakers, minted, deltaEffectiveCumSTotal)
PartnerEffectiveCumSSnapshot(epochId, vault, cumS, effectiveCumS, deltaEffectiveCumS, alpha_p, weight, rewardEarned)
...
$$\text{cumulativeRewardMinted}[\text{vault}](t) \geq \text{cumulativeRewardMinted}[\text{vault}](t-1)$$

$$\text{effectiveCumS}_p(t) = \text{cumS}_p(t) - \text{cumulativeRewardMinted}[\text{vault}] \geq 0$$
...
assert cumulativeRewardMinted[vault] >= cumulativeRewardMinted_prev[vault]  // non-decreasing
assert cumS[vault] >= cumulativeRewardMinted[vault]  // effectiveCumS >= 0
...
- CustomerVaults can be numerous → use EIP-1167 minimal proxy clones; all gas paid by partner.
...
1. **Tier multiplier on first reward:** Does the tier multiplier apply to the first reward
   ...
   (0.8× → 8% effective rate). Applying Bronze multiplier to first reward seems natural —
   but confirm.

2. **Unqualified vault EMA update:** Should `R[vault]` accumulate EMA during the unqualified
   period? Current spec says `deltaEffectiveCumS_p = 0` for unqualified vaults, so EMA builds
   from 0. Once qualified, the first growth epoch will be a clean EMA input. Confirm this is
   acceptable (vs. retroactively crediting pre-qualification growth to EMA).
...
4. **$S_{min}$ validation — confirmed USDC-direct:** Factory validates `usdcAmountIn >= S_MIN`
   where `S_MIN = 500e6` (USDC, 6 decimals). This requires that the initial buy input is always
   USDC. If partners can buy with other tokens, a secondary validation path or TWAP oracle
   is needed. Recommendation: require USDC-only initial buy input to eliminate oracle dependency.
...
8. **E_demand epoch 0 bootstrapping:** At epoch 0, `lastEffectiveCumS[vault] = initialCumS` for
   all vaults, so any initial buy sets effectiveCumS = initialCumS. Since vaults are unqualified,
   `E_demand(0)` contribution is zero. Staker pool may still receive emission if scarcity cap > 0
   and stakers exist. Confirm this bootstrap behavior is acceptable.

9. **effectiveCumS underflow guard:** If `cumulativeRewardMinted[vault] > cumS[vault]` (which
   should be impossible by invariant but checked for defensive programming), the effectiveCumS
   computation should floor at 0:
   `effectiveCumS = cumS >= cumulativeRewardMinted ? cumS - cumulativeRewardMinted : 0`.

---

## 15. v3.2 Implementation Checklist

1. **PSRE (ERC-20):** Unchanged. Mint restricted to RewardEngine.

2. **PartnerVaultFactory:**
   - Validate USDC input: `usdcAmountIn >= S_MIN` (S_MIN = 500e6, no oracle needed).
   ...
   - Set `lastEffectiveCumS[vault] = psreOut` (= initialCumS at creation).
   - Set `cumulativeRewardMinted[vault] = 0`.
   - Set `qualified[vault] = false`.
   - Deploy PartnerVault (or initialize EIP-1167 proxy).
   - CustomerVault deployment (EIP-1167 clone, linked to parent PartnerVault); gas paid by partner.
...
6. **RewardEngine (UUPS upgradeable):**
   - New storage: `mapping(address => uint256) cumulativeRewardMinted` per vault.
   - New storage: `mapping(address => uint256) lastEffectiveCumS` per vault.
   - `finalizeEpoch()`:
     - Call `snapshotEpoch()` on all active vaults → collect `deltaCumS_p`.
     - Compute `effectiveCumS_p(t) = cumS_p(t) - cumulativeRewardMinted[vault]`.
     - Compute `deltaEffectiveCumS_p = max(0, effectiveCumS_p(t) - lastEffectiveCumS[vault])`.
     - First qualification check per vault (using effectiveCumS).
     - EMA update on `deltaEffectiveCumS_p` (not TWR, not raw deltaCumS).
     - Compute `E_demand = alphaBase * sum(deltaEffectiveCumS_p) / 1e18`.
     - Compute scarcity cap, final budget.
     - Compute per-partner rewards (tier multipliers: Bronze 0.8×, Silver 1.0×, Gold 1.2×).
     - Accumulate `owedPartner[vault]`.
     - Update `cumulativeRewardMinted[vault] += reward_p` AFTER computing reward.
     - Update `lastEffectiveCumS[vault] = effectiveCumS_p(t)`.
     - Update vault activity counters (threshold = 52); mark inactive/reactivate as needed.
...
7. **Unit Tests:**
   ...
   - **E_demand: one qualified vault grows effectiveCumS → E_demand > 0.**
   - Wash trading: buy → transferOut → cumS unchanged → no reward; rebuy required.
   - Multiple partners: reward split proportional to weighted deltaEffectiveCumS.
   - Tier assignment on effectiveCumS-based EMA.
   ...
   - **Invariant: cumulativeRewardMinted monotonically non-decreasing.**
   - **Invariant: effectiveCumS ≥ 0 (cumS ≥ cumulativeRewardMinted).**
   - S_MIN enforcement: vault creation reverts if `usdcAmountIn < S_MIN` (500e6).
